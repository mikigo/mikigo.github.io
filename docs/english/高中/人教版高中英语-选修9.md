...
pugilismincasement (拳击；[交]装箱；围模；[轻]做箱的材料)
...
thornthrust (刺；跃进；刺进；套马索)

stickpink (刺；戳；使前冲)

run throughpoke (刺；突进)
...
close toborder on (接近于；[数]近似于)

nearbroadround ([数]近似的；大概的)
...
in factin practiceas it isin the concrete (实际上；事实上)
...
spit outregorge (吐出；使……呕吐)

shoot the cathork ([医]呕吐；吐出)

sicknessemesis ([医]呕吐；呕吐物；催吐剂)
...
reflectioncontemplation (冥想；沉思，深思)
...
energyspiritmindkagenius (灵魂；心灵；精神；鬼魂)
...
religiousblessed (神的；神圣的；宗教的；庄严的)
...
assetexcellenceefficiencymorality (美德；优点；贞操；功效)
...
magnificentgrand (高尚的；贵族的；惰性的；宏伟的)

princenobilityaristocrat (贵族)
...
kidpull a fast one (玩杂耍；欺骗；歪曲)

coloursellfox (歪曲；欺骗)

frauddeception (玩戏法；欺骗)
...
I’m happy to report that I finished my first sprint triathlon this morning!

我很高兴来汇报今早完成的第一次急速短跑三项全能运动！
...
configurationsideshelloutline (侧面；轮廓；外形；[建][地质]剖面)
...
The accountant disputes the figures you gave him .
...
someone who works in politics, especially an elected member of the governmen
...
energyvitalityspringvigorsteam (拉链；活力，精力；尖啸声，撕裂声；一种程序压缩的档案文件格式)
...
heavenlyethereal (天上的，天空的)

jinneedjinni (神仙，天堂里的居民)
...
ceiling boardingcurrent expenses (天花板；[会计]经常费用)
...
outeroutsideexternalmaterialopen (向外的；外面的；公开的；外服的；肉体的)

forthout (向外（等于outwards）；在外；显而易见地)

personexternalgarmentwithout (外表；外面；物质世界)
...
tiesnodus (（绳等的）结；节瘤，疙瘩；海里/小时（航速单位）)
...
marinesea-going ([水运]航海的，海上的；船员的)
...
Accordingly the nautical mile has been pegged at a standard length half way between the two possible extremes.

于是，标准的海里就取了赤道和两极的中间值。

**同近**

sea mileadmiralty knot (海里（合1.852公里）)
...
sectororthant ([海洋][天][数]象限；[水运]象限仪；四分之一圆)
...
accuratestrictmathematicalrigiddecided (精确的；明确的；严格的)
...
a tool for measuring angles between stars in order to calculate the position of a ship or aircraft
...
I can see all the traditional signalling flags neatly stowed on shelves on the bridge—so neatly, in fact, that I suspect they might never have been used, together with the sextant, the flares.

我可以看见所有传统的信号旗整齐地装载在桥架上-如此整洁，事实上，我怀疑它们和六分仪及闪光信号一样，可能从未被用过。
...
bound onsurprise attack (猛扑；突然袭击；突然下降；飞扑)

surprisesubduction (猛扑；俯冲；突然袭击)
...
leavingstartingviolationbreachquit (离开；出发；违背)
...
calculationcomputationvaluationcount ([数]计算；清算；算帐)

computingcalculatingrelyingfeelingfiguring ([数]计算；认为；指望（reckon的ing形式）)
...
aspirationlonging for (渴望；口渴；热望)
...
harryworrytrouble (撕破，撕裂；折磨；使分裂；使精神不安)
...
Denmark also has 2 autonomous provinces – the Faroe Islands and Greenland.
...
gnarhuff and puff (咆哮；吼叫；喧闹)

yell outhuff and puff (咆哮；呼喊；使……轰鸣)
...
在澳大利亚，气候变化是政治足球，在堪培拉的权力游戏中被踢来踢去。
...
然而，还需做进一步研究以确定该发现是否适用于有类似疟疾流行病的亚洲和大洋洲。
...
来自美洲、欧洲、亚洲、非洲、中东和大洋洲的大约100名临床医生、科学家以及公共卫生专业人员介绍了取得的结果和经验。
...
imposition of taxtaxe (课税，征税；[税收]税款)
...
nationalitycivil status ([法]公民身份，公民资格；国籍；公民权)
...
visitormigratory bird (候鸟；移居者；随季节迁移的民工)

ragamuffinlynomadic (移居的；流浪的)
...
“能找到这样的遗址我们真是太幸运了，”来自澳洲阿德莱德大学的主要研究员沃夫冈·哈克说到。
...
slowdull ([材]生锈的，腐蚀的；铁锈色的，锈色的；迟钝的)
...
traditionsuccessioninheritancelegacy (遗产；传统；继承物；继承权)
...
nativeoriginalprimitiveindigenous (土著的；[生物]原始的)
...
Japanese children grow up with warnings not to waste a single grain of rice, out of respect for farmers’ labor.

日本的孩子在很小的时候就受到不能浪费每一粒粮食的教育，这是出于对农民的尊重。
...
seeddistributespread (播种；散布；使密布)
...
Bachelor of Arts/Science/Education et
...
cottonidentifysquareaccordmeet with (符合，一致；相应；通信)
...
persuade sb. to doconvince sb. to do sth.get round sbpersuade sb. into doing sth. (说服某人做某事)
...
a venomous snake, insect etc produces poison
...
toxicpoisonousvicious (有毒的；恶毒的；[动]分泌毒液的；怨恨的)
...
Light in the application of anti-venom serum or hematological recovery time was not significant (P> 0.05);

轻型在应用抗蛇毒血清与否血液学恢复时间上无统计学意义（P>0.05）；
...
if something paralyzes you, it makes you lose the ability to move part or all of your body, or to feel it

**例句**

Her legs were partly paralyzed in the crash.
...
英文例句:The thief snatched her purse and ran.
...
in the midst ofamidst (在…之中；在…当中（等于among）)
...
A pollinator is something that pollinates plants, especially a type of insect.
...
trace back tobe traced to (追溯到；从…开始有)
...
kellegbacker patron ([船]锚；抛锚停泊；靠山；新闻节目主播)

fixnail (抛锚；[机]使固定；主持节目)
...
take the edge offfeather out (减弱，减少；放下，降下；贬低)

bring downplay down (降低；减弱；跌落)

juniorinferior (下游的；下级的；下等的)
...
stave offtime delay (延缓，延迟；延缓发作)
...
a large stone building with four  triangular walls that slope in to a point at the top, especially in Egypt and Central America
...
memorialcenotaph (纪念碑；历史遗迹；不朽的作品)
...
corruptiondecomposition ([植保]腐烂；腐败；腐坏)

fall into decayblet ([植保]腐烂；腐败；堕落)

corrupteat out ([植保]使腐烂；[木]使腐朽；使堕落)

ratsfiddlesticks (（表示厌恶、蔑视、烦恼等）胡说；糟了)
...
Upon returning to the hive, they deposit the nectar into pockets, called honeycombs, in the hive’s walls. It is from here that we get honey.
...
严密来说，说蝙蝠有“雷达”系统是错误的，因为他们并不使用无线电波。
...
millerdeathwatch beetle ([昆]蛾；蛀虫)
...
slowblunt (钝的；迟钝的；无趣的；呆滞的；阴暗的)

relaxbenumb (使迟钝；使阴暗；缓和)

to cut downfall off (减少；变迟钝)
...
pear-shapedmellow (圆润的；有果味的；果实状的)
...
advertising boardadvertisement board ([经]广告牌；布告板)

recommendpresentpublicize (宣传；介绍)
...
英文例句:The country is a target of criticism for its human rights record.
...
becomeadapt to something (适合，适应；符合)
...
vectordriveway ([交]车道；小巷；[航][水运]航线)
...
valuablematching (值得的；有价值的；配得上的，相称的；可尊敬的；应…的)

out-and-outerdilly (杰出人物；知名人士)
...
establishmentcompanysocietyassociationleague ([贸易]公司；法人（团体）；社团；大腹便便；市政当局)
...
consumptionlosingpricecostdisadvantage (损失，代价；消费；开支)
...
have a dislike ofhave no relish for (不需要；不喜欢)
...
slicelaminae (薄片，纸张；[材]薄板；床单)

investhouseroofcap (覆盖；盖上被单；使成大片)
...
presidentchief executive (主席，会长；董事长)
...
evilillblackuglywicked (不道德的；邪恶的；淫荡的)
...
moral philosophymoral standards (伦理学；伦理观；道德标准)
...
prescription medicineprescription drugs (处方药)
...
reliableresponsibletriedhonestsure (可靠的；可信赖的)

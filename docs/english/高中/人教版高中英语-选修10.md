...
dare tobe bold to (敢于)

adventuretake a risk (冒险；投机)

enterpriseriskundertakingthrow (企业；风险；冒险)
...
为了让自己时刻记住以住的耻辱，他躺在柴堆上睡觉，并在吃晚饭和睡觉前都尝一下苦胆。
...
comfortablepleasanteasycozy (舒适的；惬意的)
...
weakfallenrank ([木]腐烂的；堕落的；[环境]恶臭的；虚弱的；极坏的)
...
to become black, or to make something black
...
onboardashipboard ([航]在飞机上；[军][船][交]在船上；在火车上)
...
happypleasedgladgoodbright (快乐的；愉快的；高兴的)
...
王子去这个饱受战争磨难的国家鼓舞士气的使命

**同近**

fightmartialism (士气，斗志)
...
You can select any one of them for testing.
...
prosperousflourishing (兴旺的，繁荣的；大受欢迎的)
...
plight one's trothcross my heart (发誓；咒骂)

plight one's trothtake an oath (发誓，宣誓；诅咒)
...
johnconveniencebathroom (凳子；[生理]粪便；厕所)
...
lovingdevotedfaithful (忠诚的，忠心的；忠贞的)
...
experienceddiplomatic (机智的；圆滑的；老练的)
...
remainingextraordinarytemporarysurplusresidual ([数]奇数的；古怪的；剩余的；临时的；零散的)

putwack ([数]奇数；怪人；奇特的事物)
...
uncertainmobilevariableliquidunstable (无常的；可改变的；易变的；不定的)
...
...accusations that he had been disrespectful to a police officer.

...关于他曾对一名警官无礼的控告。

**同近**

rudeoffensive (无礼的；失礼的；不尊敬的)
...
portionpartappropriate (分配；拨出；使坐落于)

parcel outto assign (分配；指定)
...
used of someone you love very much
...
belovedthe light of one's eyes (心爱的人；亲爱的)
...
beseechobsecrate (祈祷；恳求；央求)

cry onappeal to sb for sth (祈祷；请；恳求)
...
arrangesort things out (整理；把…打包)
...
artificialfalse (欺骗的；欺诈的；谎言的；虚伪的)
...
happylovelydesigningknowingpretty (狡猾的；巧妙的；可爱的)

artfulnesssubtility (狡猾)
...
offeringvictimization (牺牲；祭品；供奉)

immolatemartyr (牺牲；献祭；亏本出售)

offerdevote  to (献祭；奉献)
...
把精力放在那些重要任务上，它们将使你成名，带来长时间的回报和长时间的幸福与满足。
...
make his markcome to fame (成名；造成声誉)
...
availableemptyblankleisurehollow (空虚的；空的；空缺的；空闲的；茫然的)
...
book inbook down (登记；[专利]注册；记录；挂号邮寄；把…挂号；正式提出)

book inbook down (登记；[专利]注册；挂号)

entrypageenrollment (登记；[专利]注册；记录；寄存器；登记簿)
...
partialunfairinequitable (不公平的，不公正的；非正义的)
...
distanceintervalspacegapremove (分离，分开；间隔，距离；[法]分居；缺口)
...
英文例句: He was unwilling or unable to pay the fine.
...
highestmaximumpeaktopprincipal (最高的；至高的；最重要的)
...
essentialintrinsicinterior ([法]宪法的；本质的；体质上的；保健的)
...
freedompermissionleave (自由；许可；冒失)
...
这门课程越来越受欢迎，从想修读此课程的学生人数上即可明显看出这一点。
...
It has never been weaker as a political creed .

作为一种政治信条，它从来没有像现在这样衰微过。
...
beliefteachingdoctrinetenetcredo (信条，教义)
...
totipotentall-powerful (全能的；有无限权力的)
...
strain every nerveattempt at (竭力；企图)
...
to see or describe something before it is shown to the public
...
hereunderdown below (在下面；在底下)

basisbottombase (下面；底部)

followingnether (下面的；底层的)
...
weaknessinteresthabitlikeappetite (爱好，嗜好)

curvewindingflexuralcrooked (弯曲的；决心的)
...
a white or pink flower that grows on the surface of lakes in Asia and Africa, or the shape of this flower used in decorations
...
gettingprocurementwinning (获得物，获得)
...
watchingtake care of (趋向；注意；易于；有…的倾向)
...
fuzzydarkuncertain (模糊的；含糊的；不明确的；暧昧的)
...
listenerassembly room (听众；礼堂)

acousticaural ([生理]听觉的；耳朵的)
...
spokenverbalword-of-mouth ([药][解剖]口头的，口述的)
...
descriptiontypestylebreed (类型；流派；风俗画)
...
And 19th century literature was transformed by Anne's Agnes Grey, Emily's Wuthering Heights, and Charlotte's Jane Eyre.
...
In Jane Austin's time, when men and women lived in separate worlds, their primary attraction to each other was romantic and/or sexual.

在简奥斯汀的时代，男人和女人生活在各自的世界里，他们对于彼此的吸引只能发生在浪漫/性这方面。

This is the second book (that) I have read written by Jane Austin.
...
Charlotte Bronte used it in a story published in 1849.

夏洛蒂·勃朗特在1849年发表的故事中用到了这种说法。

An electronic-book firm is adding background noises and music to the works of Charlotte Bronte, Sir Arthur Conan Doyle and William Shakespeare in the hope of attracting younger readers.

一家电子书公司正在为夏洛蒂·勃朗特、柯南和莎士比亚的作品添加背景音乐，以期望吸引年轻读者。
...
on a large scaleby the hundreds (有效的；大批地；大规模地)
...
conflictdiscrepancy ([数]矛盾；否认；反驳)
...
talk aroundwin over (说服；（消息）传开来；走动；旅行；避免)
...
正如伟大的英国作家乔治·艾略特一个世纪以前所预言，一旦建立，这个犹太国家将“在东方的专制暴政中间像一颗明星一样发光”。
...
restrictionthresholdqualificationterminal (限制；限度；极限；追诉时效；有效期限)
...
in the name ofby the name of (以…的名义；名叫…)
...
exposuremessage (启示；揭露；出乎意料的事；被揭露的真相)
...
initialfirstoriginal (第一的；最初的)

chancellorTaoiseach (总理，首相)
...
highlightexcel at (突出；站出来；坚持到底；坚决反对)
...
traderdealer ([贸易]商人，批发商；店主)

commercialshoppy (商业的，[贸易]商人的)
...
informrat (告密；吱吱叫；侥幸成功)

opportunitychanceopeningroomoccasion (吱吱声；机会)
...
at lastin the endin detail (最后，终于；详细地)
...
questionprobeexaminequery (询问；调查；问候（等于inquire）)

questionqueryinquire of (询问；打听)

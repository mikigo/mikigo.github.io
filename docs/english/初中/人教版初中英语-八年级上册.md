...
excellentfamoussupergreatclass (奇妙的；极好的)
...
我个人知道相当多的一些有才智的人，在某些情况和场景下，是有一些愚蠢，因为我也是其中之一。

**同近**

not a fewquite a bit (不少，相当多)
...
extremelybadlyhighlygreatlyalmostnearlytoo (最；非常，极其；最多；几乎)

largemaximum (大部分的，多数的；最多的)

bulkmasses of (大部分，大多数)
...
我不知道他具体是干什么的，只知道和电脑有关。

**同近**

extremelybadlyhighlygreatlytoo (非常；有点；大约)
...
each oneeverybody (a term of address)Every Body (每个人)
...
movementdoingcampaignworkingact (活动；行动；活跃)
...
settleconditionconclude (决定；解决；判决)

willdetermine to (决定，下决心)
...
apply oneselfundertakeofferproof (试图，努力；试验；审判；考验)

experimentapply oneselfstrugglelabor (尝试；努力；试验)

effortpainstrokeproofstruggle (尝试；努力；试验)
...
applecustomerfelloweggguy ([鸟]鸟；家伙；羽毛球)
...
constructionfabricstructurebldg ([建]建筑；建筑物)

establishingaddingincreasing ([建]建筑；建立；增加（build的ing形式）)
...
There’s a world of difference (= there’s a very big difference ) between us.

我们之间有着天壤之别。

**同近**

diversityconflictdisparitydeviations (差异；不同；争执)
...
roofcoping (顶部，顶端；上部；首席；陀螺)

tower overmove beyond (高出，超越；结束；达到顶点)

better thanmove beyond (超越，超过；给…加盖；达到…的顶端)

highestsuprememaximumpeaktallest (最高的，顶上的；头等的)
...
humidmoisty ([气象][物]潮湿的；有雨的)

humiditya rainy day (雨天；湿气)
...
adequatelysufficiently (足够地，充足地)

amplitudetonplenty of (很多；充足)

adequateplenitudinous (充足的)
...
eagerdesiredanxioussickkeen (饥饿的；渴望的；荒年的；不毛的)
...
quackerdeary (鸭子；鸭肉；（英）宝贝儿；零分)

jankto dodge (闪避；没入水中)

avoidstay away from (躲避；猛按…入水)
...
But they find themselves drawn back to their elm in Central Park. Spending a night there is spiritually restorative, they said, if a bit chilly of late.

但他们还是带着疲倦的身躯回到了中央公园的橡树那儿，并说，尽管最近有点儿冷，但在这棵树上睡一夜有助于恢复精神。
...
justillfreshscarcelybarely (几乎不，简直不；刚刚)
...
completefilledabundantwholeabsolute (完全的，完整的；满的，充满的；丰富的；完美的；丰满的；详尽的)

extremelybadlytotallyentirelyhighlycompletelyabsolutely (十分，非常；完全地；整整)

integrityentirety (全部；完整)
...
perhapspossiblyprobablylike (也许；可能；大概)

possibilityprobabilityfeasibilitypotentialchance (可能性；不确定性)
...
lowestminimumsmallest (最小的；最少的（little的最高级）)
...
refusegarbagewasterubbishtrash (垃圾，废物；[船]舢板)
...
food that is not healthy, for example because it contains a lot of fat, sugar etc
...
fitnesshygienewellness ([医]健康；卫生；保健；兴旺)
...
outcomeproducteventconsequencegrade (结果；成绩；答案；比赛结果)

setaccountbreed (结果；导致；产生)
...
proportionpartportionintegralfraction ([数]百分比，百分率；部分；百分数)
...
opinionintelligenceenergymemoryspirit (理智，精神；意见；[心理]智力；记忆力)

devote one's attention toconcentrate upon (介意；专心于；照料)

take care oftake notice (介意；注意)
...
to refer back to the thing or person that you have just mentioned, or a thing or person like the one that you have just mentioned
...
alongmeanwhile (一起；同时；相互；连续地；总共)
...
go outangel of death (死亡；凋零；熄灭)

join the majorityend one's days (死，死于…)

former blockcutting block ([机]冲模，钢模；骰子)
...
anywayat all events (无论如何；不管怎样)

whereaswhileneverthelessbutyet (然而；可是)
...
immediatelydirectly (比（用于形容词、副词的比较级之后）；除…外（用于other等之后）；与其…（用于 rather等之后）；一…就（用于 no sooner等之后）)

beyondabovewithout (比；超过)
...
nearlymostvirtuallypracticallyfeckly (差不多，几乎)
...
At this stage, you begin settling for less than what you deserve.

在这个阶段，尽管你得到的比应得的要少，但你还是勉强接受。
...
coremainheartkernelessential (要点；得分；[语]标点；[机]尖端)

direct tofall on (指向；弄尖；加标点于)

make knownspeak of sth. (表明；指向)
...
elderseniority (长辈；较好者；打赌的人（等于bettor）)

improveovercomeexceedcap (改善；胜过)
...
smoothlyundergroundprivatelysecretly (安静地；秘密地；平稳地)
...
gamewarmatchtournamentstruggle ([生态]竞争；比赛，竞赛)
...
obviouslyeasilyno doubtbroadly (清晰地；明显地；无疑地；明净地)
...
purchasebear off (赢得；在…中获胜；劝诱)

arrivemake good (赢；获胜；成功)
...
虽然帕斯卡知道这个试验很危险，但他仍然进行下去。

**同近**

neverthelessnot but that (可是，虽然；不过；然而)

althoughwhileasalbeit (虽然；尽管)
...
disturbanceworries (关怀；照料；谨慎；忧虑)

tendattendthink of (照顾；关心；喜爱；顾虑)
...
acutebadgravesolemn (严肃的，严重的；认真的；庄重的；危急的)
...
inevitablerequiredessentialcertainautomatic ([数]必要的；必需的；必然的)
...
If you're doing iterative development, what you want to measure in the Elaboration phase will be different from what you measure in the Construction phase.

如果你正在做迭代开发，什么是你想在细化阶段测量的将与什么是你想在构建阶段测量的是不同的。
...
differ frombe distinct from (与…不同，不同于)
...
produceto publish (出版，生产；使显示；说出)
...
degreeclassesstagephaserate (年级；等级；成绩；级别；阶段)
...
中文例句：The talks will continue until a conclusion is reached.

会谈将一直进行下去，直到得出结论.

**同近**

gorangearrive at (达到；延伸；伸出手；传开)

accomplishaffectextendinfluenceimpact of (达到；影响；抵达；伸出)

extentboundaryregionscopespectrumterritoryrange (范围；延伸；河段；横风行驶)
...
assistancecraftaidindexfinger ([解剖]手，手艺；帮助；指针；插手)

encouragecarrysecondbacktransfer (传递，交给；支持；搀扶)
...
come in contact withmake contact with (接触；触动；使轻度受害)

deal withto relate to (触摸；涉及；接近；提到)

contactfeelinglittle (接触；[生理]触觉；格调；少许)
...
courageaffectioncoremainfeeling ([内科][解剖]心脏；感情；勇气；心形；要点)

encouragebranduphold (鼓励；铭记)
...
truthrealityactuality (事实；实际；真相)
...
in practicein realityas it isin facein nature (事实上，实际上)
...
make something separate into two or more pieces, for example by hitting it, dropping it, or bending it
...
participate inportion (分享，分担；分配)
...
ceaselessunceasing (大声的，高声的；不断的；喧吵的)

aloudringingly (大声地，高声地，响亮地)
...
junior schoolelementary schools (小学)
...
feedbackadvicenoticeknowledge ofcognition ([计][通信]信息，资料；知识；[图情]情报；通知)
...
dramasurgerystagecinema (电影院，戏院，剧场；戏剧；手术室)
...
familiarnearcompact (紧密的；亲密的；亲近的)

concludepull the plug (关；结束；使靠近)

pull the plugdraw to an end (关；结束；关闭)

cheek by jowlcompactly (紧密地)

completionterminationfinishended up (结束)
...
pessimalbaddest (最差的，最坏的；最不利的；效能最低的)
...
conditionconclude (选择，决定)

opt forto make a choice (选择，挑选)
...
the police, hospitals, fire service, and organizations that provide basic things such as water, gas, or electricity

基础服务部门

**同近**

ceremonywaiting (服务，服侍；服役；仪式)

durablewearproof (服务性的；耐用的；服现役的)

maintainunkeep (维修，[计]检修；保养)
...
playtake a part of (扮演；装作，举动像)

operatebear oneselfconduct oneself (行动；扮演，充当；表现，举止；假装，演戏；起作用，见效)

behaviordealingactiongoingbill (行为，行动；[法]法令，法案；（戏剧，歌剧的）一幕，段；装腔作势)
...
mutualcorporateusualordinarycollective (共同的；普通的；一般的；通常的)

ordinarypopulace (普通；平民；公有地)
...
Roll up, roll up! See how the magician performs his tricks!

快来看，快来看!看看魔术师怎么表演他的魔术!
...
undertakeaddresspursuehavewage (胜任；该由…负责；轮到…；从事，忙于)
...
workersmissionpartassignmentcharacter (角色；任务)
...
然后他/她会向你解释如何将这些事情结合起来，在你择业时发挥作用。
...
awardcapture ([劳经]奖品；奖赏；战利品)

assessacquireevaluatevaluerate (珍视；捕获；估价)
...
constituterecoverarrange (弥补；组成；化妆；整理)
...
badlyseverelyearnestly (认真地；严重地，严肃地)
...
attach importance tomake account of (重视；认真对待…)
...
awardeffectformgrantinspire (给；产生；让步；举办；授予)

flexibilitycurveelasticity (弹性；弯曲；伸展性)

lookmake a contribution to (捐赠；面向；有弹性；气候转暖)
...
Examples of this would be a half-hour sitcom, a one-hour drama, an episodic documentary, a how-to program such as cooking or home repair or a reality show.

包括半小时的情景喜剧，一小时的剧情片，一部插话式的纪录片，一个诸如烹饪或家居修补或真人秀的指引类节目。
...
As a TV program type, situation comedy has been in America for more than 50 years, and it developed well.
...
They larded many commercials into a ten-minute period before the soap opera.

他们在肥皂剧前的十分钟内插播了许多商业广告。
...
schemeprogrammeproject (计划；[测]平面图)

designscheduleproposemapproject (计划；设计；打算)

projectaimpropose (计划；打算)
...
中文例句：When I first arrived in New York, I was full of hope for the future.

我初到纽约时，对未来充满希望。

**同近**

wishpleasureconfidencepromiseanticipation (希望；期望；信心)

wantdesire to (希望；期望)

relydependexpectliketrust (希望；期待；信赖)
...
assure oneselfchase down (找出，查明；发现，揭发)
...
stomachgotoughabidesustain (使站立；忍受；抵抗)

stopstay (站立；立场；看台；停止)
...
to indicate that something will possibly happen or be true in the future, but you cannot be certain
...
make sport ofplay trick on (开…的玩笑)
...
civilizationplanting (文化，文明；修养；[农学]栽培)

rearfosterplantnurse ([细胞][微]培养（等于cultivate）)
...
达•芬奇的那幅闻名世界的肖像画《蒙娜丽莎》

**同近**

notedwonderfulexcellentsupergreat (著名的；极好的，非常令人满意的)
...
occurfigureoffercome out (出现；显得；似乎；出庭)
...
getfallcome (成为；变得；变成)

suitadapt to something (适合；相称)
...
someone  has a lot of money and valuable possessions
...
expensivedearwealthycostly (富有的；肥沃的；昂贵的)
...
strengthforcemusclepowerinfluence (力量；威力；势力)
...
moviemembranecinema ([电影]电影；薄膜；[感光]胶卷；轻烟)
...
evilillunfortunatetragic (不幸的；倒霉的；不吉利的)
...
spendwaste on (浪费；使沉溺于；使迷路；遗失；错过)

foundercome to naught (失败；受损失)
...
be prepared to dofeel like doing sth. (愿意做某事；为…做准备)
...
dress oneselftrick up (打扮，装饰；穿上盛装)
...
shape up or ship outgive a good account of oneself (干得好；好好干)
...
What is the best part of being a computer programmer?

成为一名计算机程序员最好的部分是什么？

What are some qualifications of a computer programmer?
...
medicalhakeem ([医]医生；博士)

repairperform maintenance (修理；窜改，伪造；为…治病；授以博士学位)
...
gineersapper ([经管]工程师；工兵；火车司机)

designstyleprojectframescheme (设计；策划；精明地处理)
...
Guitarist Django Reinhardt and violinist Stephen Grapelli are the most famous gypsy jazz artists and they both happen to play for this quintet.

吉他手莱因哈特和小提琴家斯蒂芬·格拉佩利是最著名的吉普赛爵士乐艺术家，他们都曾为法国爱乐五重奏演出。
...
aviatoramn ([航]飞行员；[水运]领航员)

to navigateexperiment with ([航]驾驶；[水运]领航；试用)
...
There were many I didn't know, and others I couldn't be sure about because they were all covered in green parasites.
...
And you have to make sure, what I always say.

所以我经常说，你要确保良好的工作环境。

Then, make sure that you and your spouse share these goals with each other.

尽可能具体地说，就是确保你和你的配偶彼此共享这些目标。

**同近**

favorknowsustainvalidate (确信；证实)
...
universityacademycampusschool (大学；学院；学会)
...
cultivationtrainingpedagogy (教育；培养；教育学)
...
drugpharmacon ([医学][药物][药]药；医学；[内科]内科；巫术)
...
itemprovisionclause (文章；物品；[法]条款；[语]冠词)
...
beamreleasemissionprojectlet go (发送，寄；派遣；使进入；发射)

to post a letter (派人；寄信)

ascending motionupward movement (上升运动)
...
settlementdecisiondeterminationpushbackbone ([物][电子][光]分辨率；决议；解决；决心)
...
capableefficientcompetent (能；[经管]有能力的；能干的)
...
have enough freedom, power, time, or money to do something
...
be capable ofbe in a position to (会；能够)
...
significanceintentionwillimportoffer (意义；含义；意图)

significanteloquent (意味深长的)
...
a statement that you make to a person in which you say that you will definitely do something or give them something
...
wishhopepleasure (许诺，允诺；希望)

pledgevouchsafe (允诺，许诺；给人以…的指望或希望)
...
thresholdopeningconceptionfreshfirst (开始；起点)

startinginitiating (开始；创建（begin的ing形式）)
...
furtherbuild up (改善，增进；提高…的价值)

gainaccelerategrow in (增加；变得更好)
...
On the morning before starting a diet, write down your starting weight.

早上开始节食前，记下你的起始体重。

**同近**

get sth downstick down (写下；减低帐面价值；把…描写成)
...
be connected withbe concerned inbe correlated withhave relation tobe related to (与…有关)
...
weaknesshabitappetiteliking (嗜好；业余爱好)
...
a magazine that appears once a week
...
If you have kids, then you can’t help but have some clutter with toys and school work.

如果你有孩子，那么你只能成天徘徊于堆积如山的玩具和学校的事情中。

The big demand is for teachers-turned-nannies, who can help the kids with increasingly demanding school work.

做过老师的保姆需求量很大，她们可以帮助孩子完成要求越来越高的学校作业。
...
undertakeacceptrecognizesidegrant (同意；赞成；承认)
...
undertakeacceptgrant (同意，和…意见一致)
...
tomorrowprospect (未来；前途；期货；将来时)

prospectiveulterior (将来的，未来的)
...
contextcircumstancesettingsurroundingcondition ([环境]环境，外界)
...
landclaydirt ([天]地球；陆地；泥土；尘世；[电]地线)
...
harmonyequilibriumquiet (和平；平静；和睦；秩序)
...
oceanlots ofwealthbodyhundred ([海洋][地理]海；海洋；许多；大量)
...
heavenheightsummitairpeak ([天]天空；顶点)

sharp increasego through the roof (踢或击高空球；把桨叶翘得过高；飞涨)
...
separationvacuumroomdistanceremove (空间；太空；距离)

keep distance (留间隔)

partition offfence out (隔开)
...
demichominine (人的；[人类]人类的)

personmankindmancreaturehomo ([人类]人；人类)
...
criticalriskythreateningprecariousperilous (危险的)
...
plantmillworkmanufactory ([工经]工厂；制造厂；代理店)
...
trust onsuppose (信任；料想；笃信宗教)

expectcountguessfindfeel (相信；认为；信任)
...
thousandlots and lots (好几百；许许多多)
...
matrixformmodel (形状；模型；身材；具体化)

moldmakemodel (形成；塑造，使成形；使符合)

to come into beingto form (形成；成形；成长)
...
interiorwithininner (里面；内部；内情；内脏)

internalinnersecretundergroundchamber (里面的；内部的；秘密的)

withinunderin (少于；在…之内)
...
" so we can look for those features.

这样我们就可以寻找那些特征。

**同近**

search forseek for (寻找)
...
reasonablefeasibleapplicablelogicalrational (可能的；合理的；合适的)

probabilityfeasibilitypotentialmaybechance (可能性；合适的人；可能的事物)
...
我只是希望能以合理的价格买到健康食品。难道这是在要求不可能的事吗？

**同近**

incorrectunbearable (不可能的；不可能存在的；难以忍受的；不真实的)
...
in or on the side of something
...
aspectrespect (方面；侧面；旁边)

support ofroot for (支持；赞助；偏袒)

undertakeencouragecarrysecondback (同意，支持)
...
possiblymaybepresumably (大概；或许；很可能)
...
newsinstructionbiddingmessageorder ([语]单词；话语；消息；诺言；命令)
...
rockswing motion (动摇；摇动；震动)

waverock (动摇；摇动；发抖)

rocking motionwafture (摇动；哆嗦)
...
a sweet drink made of milk with fruit or chocolate added

**例句**

This milk shake is a 32oz meal which infuses you with an entire day’s calorie and almost two day’s fat allowance in one drink, 2310 calories and 108g saturated fat.

一份32盎司的奶昔包含了你身体一整天所需要的卡路里和几乎两天的所需要的脂肪量-2310的卡路里和108克的饱和脂肪。
...
They're easy to grind in a blender or coffee grinder. But get seeds -- there are no lignans in the oil.

他们在搅拌机或咖啡豆的磨具很容易被磨碎，但种子如果榨成油木酚素就被破坏了。
...
mixturehybridmixcompound (（美）搅拌机；掺和者；混合物)
...
cracklaunchdrivewageopen up (打开，发动)
...
It can be store-bought plain yogurt or the last spoonfuls of your most recent homemade batch.

它可以使商店中没来的纯酸奶，或者是最后一勺你最近一批自制的酸奶。
...
improvegainaccelerate ([数]加；增加；加起来；做加法)

enhanceboostjackmounthot (增加，添加；补充说；计算…总和)
...
saliferousbrackish (咸水的；含盐的，咸味的；盐腌的；猥亵的)
...
saccharidesweet talk ([食品]糖；食糖；甜言蜜语)

colourprettify (加糖于；粉饰)
...
personagefromage (奶酪；[食品]干酪；要人)

quitcease fromdesist fromcome to a stopto stop (停止)
...
grainzea mays (（美）[作物]玉米；（英）谷物；[皮肤]鸡眼)
...
organizationrobothouse (机械，[机]机器；机构；机械般工作的人)
...
cavityhollowaperture (洞，孔；洞穴，穴；突破口)
...
kiss up tosoft-sawder (涂黄油于；讨好)

creambeurre ([食品]黄油；奶油；奉承话)
...
blockmass ([计]块；件；篇；硬币)

cementjoint (修补；接合；凑合)
...
conventionalusualclassical (传统的；惯例的)
...
fallmaturation phase (秋天；成熟期；渐衰期，凋落期)
...
passengertourist (旅行者；旅客；旅行推销员（等于traveller）)
...
stagepraiseblesscommemorate (庆祝；举行；赞美；祝贺)

rejoicewhoop it up (庆祝；过节；举行宗教仪式)
...
participatetravelcompanysort (参与；相混合；交往)

mixturehybridchaosdisorderconfusion (混合；混合物；混乱)
...
pepercapsicum ([食品][植]胡椒；[园艺]辣椒；胡椒粉)
...
inviteattendguest (招待，供应；为…服务；对…有用；可作…用)

gearhelp (服役，服务；适合，足够；发球；招待，侍候)
...
paintswing from (悬挂，垂下；装饰；绞死；使悬而未决)

trail (悬着，垂下；被绞死；悬而不决)

moratoriumpause (悬挂；暂停，中止)
...
realizeaffectgraspcontractcommunicate (赶上；抓住；感染；了解)

get up tomake it to (赶上；抓住)
...
serveattendaskcourtguest (邀请，招待；招致)

callinginvitation to (邀请)
...
undertakeadoptrecognizecontainshoulder (接受；承认；承担；承兑；容纳)

recognizegrant (承认；同意；承兑)
...
garbagewastejunkrubbishtrash (垃圾；[环境]废物)

negativeturn down (拒绝；不愿；抵制)
...
And she has fasted ever since the day before yesterday's walk, from sorrow and rage that I dispatched her out of your society under the idea of its being unacceptable.

而且自从前天散步以后她就又难过又愤怒，以致不吃不喝，就因为我把她从你身旁打发走了，认为你是不会接受她的。
...
Begin the job the day after tomorrow.
...
tendattendthink ofnurse (照顾；关心；目送)
...
respondanswer ([通信][计]回答；[法]答辩；回击)

responsedefenceanswer ([通信][计]回答；[法]答辩)
...
中文例句: 用印刷体将你的名字和地址写在一张明信片上寄给我们。

**同近**

impressionseal ([印刷]印刷业；印花布；印刷字体；印章；印记)

strike off ([印刷]印刷；[计]打印；刊载；用印刷体写；在…印花样)

bring outget out ([印刷]印刷；出版；用印刷体写)
...
graygloomy (难过的；悲哀的，令人悲痛的；凄惨的，阴郁的（形容颜色）)
...
Fred and Jeff are friends. They want to take a trip together.

弗雷德和杰夫是朋友，他们想一起去旅行。

**同近**

make a tripgo on a journey (旅游；去旅行)
...
中文例句: 您若需要什么资料，我们很乐意寄给您。

**同近**

happypleasedwillingdelighted (高兴的；乐意的；令人高兴的；灿烂美丽的)
...
beyondabovethan (没有；超过；在…外面)

outsideectad (户外；在外面；没有或不显示某事物)

outsideextraneousness (外部；外面)
...
A housewarming is a party that you give for friends when you have just moved to a new house
...
customerclientconsumer (客人，[经]宾客；顾客)

serveinviteattend (款待，招待)

boardstay over (作客，寄宿)
...
daily scheduleephemeris (日历；[天]历法；[管理]日程表)
...
sessioncongressassemblygathering (会议；会见；集会；汇合点)
...
proceedfootcompanysortmix (旅行；行进；步行；交往)

go bygo past (经过；在…旅行)

journeytourtrip (旅行；游历；漫游)
...
keep the secretSpring Scream Remix (保守秘密；不交际)
...
standardarm's-length ([数]正常的；正规的，标准的)

criterionstandardlevelprototype (正常；标准；常态)
...
besidesexcept asin addition to (除…之外)
...
surelydefinitelyok (当然；行（用于回答）；必定)
...
rileyvexed (生气的；愤怒的；狂暴的；（伤口等）发炎的)
...
agreementintelligencegraspprotocolhead (谅解，理解；理解力；协议)

intelligentcomprehensivewisebrightclever (了解的；聪明的；有理解力的)

graspingcomprehending (理解；明白（understand的ing形式）)
...
errorfaultsinblamemisunderstanding (错误；误会；过失)

err ingo wrong (弄错；误解)
...
refer to a man, boy, or male animal

**例句**

英文释义：He poured himself a whisky and sat down in the chair.

中文释义 他给自己倒了一杯威士忌，然后坐在椅子里。
...
alarmmessage (建议；劝告，忠告；通知；警告)

agree to sbmake suggestions (建议；与…商量)
...
proceduremove (步，脚步；步骤；步伐；[建]梯级)

gowalktrack (踏，踩；走)

wend one's waypad (走，迈步)
...
dutyliabilitybelieffaithconfidence (信任，信赖；责任；托拉斯)

creditbelieve in (信任，信赖；盼望；赊卖给)

hopebelieve in (信任，信赖；依靠)
...
据他的经验来看，妇女不喜欢脚上沾到水或泥。

**同近**

areer (经验；经历；体验)

sufferliveundergo (经验；经历；体验)
...
intermediaryinexhaustive (中途的；不彻底的)
...
英文例句：There’s something else I’d like to talk about as well.

...
```mermaid
graph TD
    A["静态网站<br/>&lt;script src='/counter.js'&gt;<br/>&lt;span data-pv-site&gt;"]
    A -->|HTTPS| B["Caddy / Nginx 反代<br/>/counter.js → :8000<br/>/api/* → :8000<br/>/* → 静态文件"]
    B -->|"localhost"| C["web-counter (FastAPI)<br/>counter.js | REST API<br/>Dashboard | Rate Limiter<br/>SQLite (WAL)"]
```
...
```mermaid
graph LR
    A[push main] --> B[GitHub Actions 构建]
    B --> C[gh-pages 分支]
    C --> D[sync-to-gitee 全分支镜像]
    D --> E[服务器 crontab git pull]
    E --> F[Caddy serve]
```

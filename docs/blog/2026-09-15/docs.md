...
- 你签字同意（点"允许"）。
- 工作人员给你一个通行证（返回一个文件描述符 fd 和 restore_token）。
...
不。Portal 设计了 **persist_mode**（持久模式）和 **restore_token**（恢复令牌）来解决这个问题。
...
- [xdg-desktop-portal handle_token 修复 #1549](https://github.com/flatpak/xdg-desktop-portal/issues/1549)
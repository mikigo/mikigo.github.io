...
在对象访问 age 的时候，首先是进入了 `__get__` 方法，因为先打印了 get\_instance 和 get\_owner，instance 是 Mikigo 实例对象，也就是 mi，owner 是 Mikigo 类对象。
...
你看，确实很简洁哈，my\_lb 不是函数名哈，有函数名它就不是匿名函数了，而是函数对象，咱可以调用它。

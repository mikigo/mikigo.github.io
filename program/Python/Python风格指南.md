...
【强制】 类内部的私有方法以\_开头，单独定义的私有方法以\_\_开头。

【强制】 使用全小写字母和\_隔开，命名定义能体现方法的功能，例：get\_conf。
...
【强制】 使用with打开文件，不能使用with关键字的句柄，可以自定义，实现\_\_enter\_\_和\_\_exit\_\_方法，同时还可以是用contextlib.closing
...
全局常量名： GLOBAL\_CONSTANT\_NAME

全局变量名： global\_var\_name

实例名： instance\_var\_name

函数参数名： function\_parameter\_name

局部变量名： local\_var\_name
...
【强制】用单下划线\_开头表示模块变量或函数是protected的(使用from module import \*时不会包含)。

【强制】用双下划线(\_\_)开头的实例变量或方法表示类内私有。
...
【推荐】在字典循环的时候如果只需要value，可用\_作为占位符占据key的位置。

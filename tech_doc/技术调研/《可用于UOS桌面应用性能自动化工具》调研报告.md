...
训练成功后，我们可以在本地看到生成了对应的keras\_model.h5文件，如图12所示：
...
| psnr\_threshold | 利用 psnr 进行增强型的检测，设定后，它将对被认为stable的区间进行二次检测。例如，设定为0.5时，稳定区间的条件将变为：ssim > 0.95 and psnr > 0.5 | None   | (0,1)    |
...
| compress\_rate  | 影响保存图片的缩放倍数，影响我们分析最后结果。默认为0.2，即将图片缩放为0.2倍 | 0.2    | (0, +∞)  |
...
通过我们预先写好的代码来对uos\_demo.mp4进行分帧。分帧结果如图16所示：
...
同时，为了不影响我们分析结果，我们将compress\_rate参数调整到了0.4，方便我们对图像进行分析。

利用以上参数，我们对uos\_test.mp4进行测试。最终生成报告文件report.html
